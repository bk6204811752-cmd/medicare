...
import { getB2BSalesSummary, getB2BSalesTrend } from "@/lib/stockist-db";
    ...
    <div className="space-y-6 max-w-full overflow-x-hidden">
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_380px] min-w-0 w-full overflow-hidden">
        ...
        <div className="space-y-6 min-w-0 w-full overflow-hidden">
          ...
          <section className="glass-card p-4 sm:p-5 min-w-0 w-full overflow-hidden">
            <h2 className="font-display text-base sm:text-lg font-semibold text-med-navy mb-4">Stockist Tools</h2>
        ...
        <div className="space-y-6 min-w-0 w-full overflow-hidden">
          ...
          <section className="glass-card p-4 sm:p-5 min-w-0 w-full overflow-hidden">
            <div className="flex items-center justify-between mb-3">
              <h2 className="font-display text-base font-semibold text-med-navy">Distribution Low Stock</h2>
              <span className="rounded-full bg-orange-100 px-2.5 py-0.5 text-xs font-bold text-orange-600">{lowStock.length}</span>
            </div>
            {lowStock.length === 0 ? (
              <p className="text-sm text-slate-400 py-4 text-center">Wholesale warehouse well-stocked ✓</p>
            ) : (
          ...
          <section className="glass-card p-4 sm:p-5 min-w-0 w-full overflow-hidden">
            <div className="flex items-center justify-between mb-3">
              <h2 className="font-display text-base font-semibold text-med-navy">Batch Expiry Alerts</h2>
              <span className="rounded-full bg-red-100 px-2.5 py-0.5 text-xs font-bold text-red-600">{expiry.length}</span>
            </div>
            {expiry.length === 0 ? (
              <p className="text-sm text-slate-400 py-4 text-center">All distribution lots safe ✓</p>
            ) : (
              <div className="space-y-2.5 max-h-56 overflow-y-auto pr-1">
