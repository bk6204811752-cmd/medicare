...
export function PartiesClientDashboard({ initialParties, routes }: PartiesClientDashboardProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`grid gap-6 min-w-0 w-full transition-all duration-300 ${selectedParty ? "xl:grid-cols-[1fr_640px]" : "lg:grid-cols-[1fr_360px]"}`}>
                  ...
                  {filteredParties.map((party) => {
                    ...
                    return (
                      <tr 
                        ...
                        className={`hover:bg-emerald-50/20 cursor-pointer transition-colors group ${isSelected ? "bg-emerald-50/30 border-l-2 border-l-emerald-500" : ""}`}
                      >
        ...
        {selectedParty ? (
          <div className="glass-card overflow-hidden flex flex-col animate-fade-in min-h-[600px]">
            {/* Panel Header */}
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-5">
              ...
              {activeTab === "credit" && (
                <div className="space-y-5">
                      ...
                      <button key={item.key} type="button" onClick={item.toggle}
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-[9px] text-slate-450 font-semibold">{item.desc}</p>
                          </div>
                        </div>
                  ...
                  >
                    ...
                    <h4 className="text-xs font-extrabold text-slate-450 uppercase tracking-wider">📝 Adjust Credit Metrics & Routing</h4>
                ...
                <div className="space-y-3">
                  ...
                  {ledgerLoading ? (
                    <div className="flex flex-col items-center justify-center py-16 text-center space-y-2">
                      ...
                      <p className="text-xs text-slate-450 font-bold">Loading order history...</p>
                    </div>
                                    ...
                                    <tbody className="divide-y divide-slate-50">
                                      {row.items.length > 0 ? row.items.map((item, idx) => (
                                        <tr key={idx} className="hover:bg-slate-50/30">
                                          ...
                                          <td className="px-3 py-2.5 font-mono text-slate-500">{item.hsnCode || "—"}</td>
                                          <td className="px-3 py-2.5 text-slate-500 max-w-[100px]">
                                            <span className="truncate block">{item.manufacturer || "—"}</span>
                                          </td>
                                          <td className="px-3 py-2.5 text-slate-500">{item.mfgDate || "—"}</td>
                                          <td className="px-3 py-2.5 text-slate-500">{item.expiryDate || "—"}</td>
                                          <td className="px-3 py-2.5 text-slate-500">{item.packSize || "—"}</td>
                                          ...
                                          <td className="px-3 py-2.5 text-right font-mono text-slate-500">{item.mrpPaisa ? formatCurrency(item.mrpPaisa) : "—"}</td>
              ...
              {activeTab === "payments" && (
                <div className="space-y-4">
                  ...
                  {receiptRows.length > 0 && (
                    <div>
                      <h4 className="text-xs font-extrabold text-slate-400 uppercase tracking-wider mb-3">💰 Collection Receipts Received</h4>
                          ...
                          <tbody className="divide-y divide-slate-50">
                            {receiptRows.map((row) => (
                              <tr key={row.id} className="hover:bg-slate-50/30">
                                ...
                                <td className="px-3 py-2.5 font-mono text-[10px] font-bold text-slate-700">{row.refNo}</td>
                                ...
                                <td className="px-3 py-2.5 text-right font-mono font-black text-emerald-700">{formatCurrency(row.creditPaisa)}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
              ...
              {activeTab === "ledger" && (
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h4 className="text-xs font-extrabold text-slate-400 uppercase tracking-wider">📋 B2B Running Ledger (Khata)</h4>
                    <span className="text-[10px] font-extrabold text-slate-505 bg-slate-100 px-2 py-0.5 rounded">{ledger.length} entries</span>
                  </div>
                          ...
                          {ledger.map((row) => (
                            <tr key={row.id} className="hover:bg-slate-50/50 transition-colors">
                              ...
                              <td className="px-3 py-2.5 max-w-[200px] break-words">
                                ...
                                {row.notes && (
                                  <p className="text-[9px] text-slate-400 italic mt-1">Note: {row.notes}</p>
                                )}
                              </td>
